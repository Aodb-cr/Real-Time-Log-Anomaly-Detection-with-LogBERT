...
def fixed_window(line, window_size, adaptive_window, seq_len=None, min_len=0):
    ...
    arr = np.array(line, dtype=object)

    # if time duration exists in data (2 columns)
    if arr.ndim == 2 and arr.shape[1] == 2:
        tim = arr[:, 1].astype(float)
        toks = arr[:, 0]
        # the first time duration of a session should be 0
        if tim.size > 0:
            tim[0] = 0.0
        line = toks
    else:
        # Flatten tokens to a 1-D array robustly without collapsing to scalar
        if arr.ndim == 2:
            if arr.shape[1] == 1:
                toks = arr[:, 0]
            elif arr.shape[0] == 1:
            ...
            toks = np.array([arr.item()], dtype=object)
        line = toks
        # if time duration doesn't exist, then create a zero array for time
        tim = np.zeros(line.shape, dtype=float)

    logkey_seqs = []
    time_seq = []
    for i in range(0, int(len(line)), int(max(1, window_size))):
